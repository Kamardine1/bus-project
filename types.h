...
typedef enum t_month {JANVIER = 1, FEVRIER , MARS, AVRIL , MAI ,JUIN ,JUILLET,AOUT,SEPTEMBRE ,OCTOBRE , NOVEMBRE,DECEMBRE} t_month;
...
typedef struct T_station { //représente UN TRONCON (entre deux arrêts) ou UN ARRET (cad une station de la ligne de bus)
    ...
    int coutMaintenance;                //en Kilo euro, une valeur entre 10 et 100
    t_date dateDerniereMaintenance;      //t_date definis, doit permettre de stocker une date jour/mois/année, avec une date aléatoire
...
void afficheLigneBus( TligneBus *lignebus);
