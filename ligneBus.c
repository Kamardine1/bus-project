...
Tstation *creeArret( int posX, int posY, char* nomStation, int idStation){
    ...
    newStation->coutMaintenance=RandomCoutMaintenance();
    newStation->dateDerniereMaintenance=RandomDate();
...
Tstation *creeTroncon(int idLigneBus, Tstation* depart, Tstation *arrivee){
    ...
    newStation->coutTemps = DistanceBetween2Station(depart,arrivee);
    newStation->coutDistance = DistanceBetween2Station(depart,arrivee);
    return newStation;
}
